...
uniform float densityFactor = 0.22;
...
void main(){
	...
	vec4 bg = vec4(.2,0.0,0.5,1.0); //texture(sky, fragCoord/iResolution); CHANGE HERE
	vec3 red = vec3(1.0);

	//vec4 bg = mix( mix(red.rgbr, vec4(1.0), SUN_DIR.y), bg, pow( max(cubeMapEndPos.y+0.1, .0), 0.2));
	int case_ = 0;
	...
	//cloudDistance_v = v;
	//v.a = 1.0;
	float cloudAlphaness = threshold(v.a, 0.4);
